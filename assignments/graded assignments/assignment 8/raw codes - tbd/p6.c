...
void transposition_cipher_decrypter(char *string, signed *key, signed *length , signed *rows, signed *columns);
...
signed main(void)
{
    ...
    signed columns = key;
    transposition_cipher_decrypter(string, &key, &length, &rows, &columns);
